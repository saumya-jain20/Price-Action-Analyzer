...
async def deviation(df: pd.DataFrame, left_ind: int, ind: int, right_ind: int) -> bool:
    ...
    first_change = df["High"].iloc[ind] - df["Low"].iloc[right_ind]
    second_change = df["Close"].iloc[ind] - df["Close"].iloc[left_ind]
...
async def get_impulses(df: pd.DataFrame) -> list:
    result = []
    order = 18
            ...
            if r_check:
                for k in range(7, 3, -1):
                    ...
                    if l_check:
                        ...
                        if dev_check:
                            flag = True
                            first_ind = df[i : i + j + 1]["Low"].idxmin()
